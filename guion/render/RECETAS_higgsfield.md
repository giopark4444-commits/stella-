# RECETAS PARA GENERAR EN HIGGSFIELD (manual) — Locaciones del Arranque
...
> shelf) y genera. El estilo hand-painted ya va incrustado en cada prompt (no hace falta el token).

---

## 1) `@erdia_dorada` — Ciudad/planeta en su gloria
**Adjunta:** tus láminas de **pradera verde con ruina** + **ciudad-ruina colosal invadida**
(las que me mostraste como 2 y 3).
**PROMPT:**
```
A peaceful post-war alien civilization seen in daylight: lush green rolling hills and
...
background art in the style of classic hand-painted cel animation films (Nausicaä, Castle in the Sky),
...
## 2) `@palacio_pasillo` — Santuario del Núcleo Profundo (pasillos)
**Adjunta:** tu **lámina "PASILLOS – SANTUARIO DEL NÚCLEO PROFUNDO"** (referencia directa).
**PROMPT:**
```
Serene transit corridors of a sacred deep-core sanctuary: noble teal-green stone walls and
warm sandstone arches in soft organic curves, a vertical beam of luminous core-light
...
the style of classic hand-painted cel animation films, gouache-and-watercolor texture, soft cel-shaded
...
## 3) `@taller_naio` — Sala de la Fuente (el Orbe)
**Adjunta:** tu **lámina del Santuario** (misma familia visual que los pasillos).
**PROMPT:**
```
A tall sanctuary chamber of the deep core in teal-green stone and warm sandstone, half-
...
## 4) `@tierra_muerta` — Interior muerto del planeta
**Adjunta:** (ninguna directa; opcional, una de paleta oscura). Texto define todo.
**PROMPT:**
```
The dead interior of a planet: an immense underground cavern of long-dead biomechanical
...
classic hand-painted cel animation films, gouache-and-watercolor texture, soft cel-shaded forms, somber
...
### Consejo de uso
- Genera **2–4 variantes** de cada una y te quedas con la mejor.
- Cuando tengas la elegida, súbela/guárdala como **referencia maestra** de esa locación
  (así las tomas de video de esa locación nacen todas de la misma imagen).
- Pásame cuál quedó (o su media_id) y la registro en `BIBLIA_referencias.md` como `ref_id`.
