# DESGLOSE DE PRODUCCIÓN — STELLA FUGAZ (Película 1)

> Script breakdown completo del guion `PELI1_GUION_COMPLETO_actual.md` (33 escenas).
> Por cada escena: **LOC** (locación) · **CAST** (personajes) · **PROPS** · **NAVES/VEHÍCULOS** ·
> **CRIATURAS** · **VFX**. Los `@tags` enlazan con `BIBLIA_referencias.md`.
...
# A · DESGLOSE ESCENA POR ESCENA

## 起 KI

### ESC 1 — EXT. TIERRA MUERTA
- **LOC:** ✓ `@tierra_muerta`
- **CAST:** —
- **PROPS:** —
- **VFX:** ⛔ `@fx_brote` (brote verde abriéndose) · ⛔ `@fx_gota` (gota de agua / latido)

### ESC 2 — EXT. CIUDAD DE ERDIA + descenso al interior
- **LOC:** ✓ `@erdia_dorada` · ✓ `@tierra_muerta` (el descenso por la fractura)
- **CAST:** ⛔ `@pueblo_naiel_dorado` (figurantes: 2 ancianos, niños, cuidador de la fuente, niña pequeña)
- **PROPS:** ✓ `@rio_luzagua` · ✓ `@fuente_plaza` · ⛔ `@prop_cuenco` (cuenco de luz líquida)
- **VFX:** luz-agua, hora dorada, título en pantalla, transición fractura→interior muerto
...
- **VFX:** Orbe inclinándose hacia la palma, palpitar azul a destiempo

---

## 承 SHŌ

### ESC 5 — INT. PALACIO · SALA DE LA FUENTE (la caída)
...
- **CAST:** ✓ `@stella_14` · ✓ `@gix`
- **PROPS:** ✓ `@orbe` · ✓ `@brazalete`
- **NAVES:** ⛔ `@capsula_huida` (cápsula de escape)

### ESC 7 — EXT. ESPACIO · ÓRBITA DE ERDIA
- **LOC:** ⛔ `@espacio_orbita`
- **CAST:** ✓ `@stella_14` · ✓ `@gix`
- **PROPS:** ✓ `@orbe`
...
- **VFX:** ventanal al mar de estrellas

*(SALTO: 8 años)*

### ESC 10 — EXT. PLANETA ÁRIDO · RUINAS
- **LOC:** ⛔ `@planeta_arido`
- **CAST:** ✓ `@marek` · ⛔ `@guerrera_misteriosa` (cara cubierta — identidad oculta) · ✓ `@g44` · ⛔ `@resistencia_figurantes`
- **VFX:** polvo rojo, combate

### ESC 11 — EXT. OTRO PLANETA · CAMPO DE BATALLA
- **LOC:** ⛔ `@planeta_batalla`
- **CAST:** ⛔ `@stella_22` (adulta, pelo blanco con ceniza, guerrera) · ✓ `@noah` · ⛔ `@prisioneros` · `@tripulacion_nodriza`
- **PROPS:** ✓ `@orbe` (usado como arma: látigos/escudos de luz) · ⛔ `@prop_cadenas`
- **VFX:** Orbe canalizado en combate

### ESC 12 — INT. NAVE NODRIZA · BODEGA / NIDO DE LA CUADRILLA
- **LOC:** ✓ `@nave_nodriza` (sub: bodega — ⛔ `@nodriza_bodega` ficha propia)
- **CAST:** ⛔ `@stella_22` · ✓ `@brog` · ✓ `@nima` · ✓ `@noah` · ✓ `@vosk` (mayor, callado — variante menor)

### ESC 13 — EXT. PLANETA-BOSQUE · CLARO (noche)
- **LOC:** ⛔ `@planeta_bosque`
...
- **LOC:** `@nodriza_bodega`
- **CAST:** ⛔ `@stella_22` · ✓ `@brog` · ✓ `@nima` · ✓ `@noah`

---

## 転 TEN

### ESC 17 — INT. RESISTENCIA · REFUGIO SUBTERRÁNEO
- **LOC:** ⛔ `@refugio`
- **CAST:** ⛔ `@vera_mayor` · ✓ `@marek` · ⛔ `@korin` · ⛔ `@lessa` · ⛔ `@refugiados` (niños, heridos, viejos)

### ESC 18 — INT. PALACIO PODRIDO · CORREDORES
- **LOC:** ⛔ `@palacio_podrido`
- **CAST:** ⛔ `@vera_mayor` · ✓ `@marek` · ⛔ `@korin` · ⛔ `@lessa` · ⛔ `@naio_esclavo`
- **PROPS:** ⛔ `@prop_cadenas` · ⛔ `@set_consola` · neblina tóxica
- **VFX:** ⛔ `@fx_neblina` (neblina tóxica del palacio podrido)
...
### ESC 21 — INT. NAVE PEQUEÑA · CABINA
- **LOC:** ⛔ `@nave_pequena_cabina`
- **CAST:** ✓ `@stella_adulta` (aquí se tiñe el pelo de oscuro) · ✓ `@noah` · ✓ `@brog` · ✓ `@nima`
- **PROPS:** ⛔ `@prop_tinte_espejo` (tinte + espejo)
- **NAVES:** ⛔ `@nave_pequena` (interior)

### ESC 22 — EXT. ERDIA PODRIDA · POBLADO DE LA RESISTENCIA
- **LOC:** ✓ `@erdia_podrida` (sub: poblado)
- **CAST:** ✓ `@stella_adulta` · ✓ `@noah` · ✓ `@brog` · ✓ `@nima` · ⛔ `@korin` (mayor) · ⛔ `@lessa` (mayor) · ⛔ `@rebelde_hostil` · ⛔ `@multitud_erdiana`

### ESC 23 — EXT./INT. ERDIA PODRIDA · PREPARATIVOS (montaje)
- **LOC:** ✓ `@erdia_podrida`
- **CAST:** ✓ `@stella_adulta` · ⛔ `@korin` · ⛔ `@lessa` · ⛔ `@pueblos_oprimidos` · ⛔ `@faccion_colaboracionista`
- **PROPS:** ⛔ `@set_mapa_palacio`
...
### ESC 26 — INT. SALA DEL TRONO · EL COMBATE
- **LOC:** ✓ `@sala_trono`
- **CAST:** ✓ `@stella_adulta` · ✓ `@selka_general`
- **PROPS:** ✓ `@orbe` · ✓ `@sable_selka`
...
- **LOC:** ✓ `@sala_trono`
- **CAST:** ⛔ `@rey_humano` · ✓ `@marek` · ✓ `@selka_general` · ✓ `@stella_adulta` (en coma, suelo) · ⛔ `@korin` · ⛔ `@lessa`
- **PROPS:** ✓ `@orbe` · ⛔ `@set_pedestal_fuente`
- **VFX:** ⛔ `@fx_inversion_luz` (luz-agua succionada, oscurecimiento, cataclismo en las raíces)
...
- **LOC:** ✓ `@tierra_muerta`
- **CAST:** —
- **VFX:** ✓/⛔ `@fx_brote` (el brote verde — rima con ESC 1)

---

# B · REGISTRO MAESTRO DE ASSETS (consolidado)
...
| `@rey_humano` | Forma humana regia del Rey | 5, 27, 28 | ⛔ |
| `@vorthan_real` | El Parásito, forma real colosal | 29, 30 | ✓ ficha (falta ref_id) |

## B.2 · PERSONAJES SECUNDARIOS / FIGURANTES
| Asset | Escenas | Ref |
|---|---|---|
| `@pueblo_naiel_dorado` (ancianos, niños, cuidador) | 2 | ⛔ |
...
**⛔ por crear (12):** `@conductos_hangar` · `@espacio_orbita` · `@nodriza_puente` · `@nodriza_bodega` · `@planeta_arido` · `@planeta_batalla` · `@planeta_bosque` · `@refugio` · `@palacio_podrido` · `@sala_entrenamiento` · `@nave_pequena_cabina` · `@nave_pequena_medica`

## B.4 · PROPS
**✓ con ref (5):** `@orbe` · `@brazalete` · `@rio_luzagua` · `@fuente_plaza` · `@sable_selka`
**⛔ por crear (clave):** `@foto_familia` · `@tanque_soporte`
**⛔ set dressing / menores:** `@prop_cuenco` · `@prop_maceta` · `@set_campo_contencion` · `@set_proyeccion` · `@set_mapa_estelar` · `@set_mapa_palacio` · `@set_pedestal_fuente` · `@set_consola` · `@prop_carro_luz` · `@prop_cadenas` · `@prop_tinte_espejo` · `@prop_arma_marek`
...
| `@nave_vorthan` | Nave nodriza colosal del Parásito | 31 |

## B.6 · CRIATURAS
`@gix` ✓ · `@vorthan_real` ✓(ficha)

## B.7 · VFX / MOTIVOS RECURRENTES (tratar como assets para consistencia)
| Asset | Qué | Escenas |
|---|---|---|
...
# D · CONTEO Y PRIORIDADES

- **Personajes:** 19 principales (con variantes) + 13 grupos de figurantes/robots.
- **Locaciones:** 19 (7 listas, **12 por crear**).
- **Props:** 7 con ref + ~14 por crear · **Naves:** 5 (todas por crear) · **VFX:** ~11 motivos.
- **Total de fichas de referencia que faltan: ~50 assets ⛔.**

**Orden sugerido de creación (sigue el orden de rodaje del guion):**
1. **Secuencia de huida (Ki→Shō):** `@conductos_hangar`, `@capsula_huida`, `@espacio_orbita`, `@naves_rey`, `@fx_cascara_gix`, `@fx_estrella_fugaz`, `@rey_humano`, `@g44`.
2. **Bloque nodriza:** `@stella_22`, `@nodriza_puente`, `@nodriza_bodega`, `@tripulacion_nodriza`, `@foto_familia`.
3. **Bloque resistencia/Ten:** `@vera_mayor`, `@naio_esclavo`, `@korin`, `@lessa`, `@marek`(ref_id), `@palacio_podrido`, `@sala_entrenamiento`, `@refugio`, `@guardia_vacia`.
4. **Clímax/Ketsu:** `@selka_manca`, `@vorthan_real`(ref_id), `@fx_metamorfosis_vorthan`, `@fx_inversion_luz`, `@nave_pequena`, `@nave_vorthan`, `@tanque_soporte`, `@fx_latido_brazalete`.
